 ...
 * accuracy. This
 * implementation highly depends on zh_CN ICU collation objectMap and must be always
 * synchronized with
 * ICU.
 *
...
public class HanziUtils {
    ...
    public static String getPinYin(String input) {
        ...
        ArrayList<Token> tokens = HanziUtils.getInstance().get(input);
    ...
    private static final String TAG = "HanziUtils";

    // Turn on this flag when we want to check internal objectMap structure.
    ...
    private static HanziUtils sInstance;
    ...
    protected HanziUtils(boolean hasChinaCollator) {
        mHasChinaCollator = hasChinaCollator;
    }


    public static HanziUtils getInstance() {
        synchronized (HanziUtils.class) {
            if (sInstance != null) {
                return sInstance;
            }
            // Check if zh_CN collation objectMap is available
            ...
            for (int i = 0; i < locale.length; i++) {
                if (locale[i].equals(Locale.CHINA)) {
                    ...
                    sInstance = new HanziUtils(true);
                    return sInstance;
                }
            }
            Log.w(TAG,
                    "There is no Chinese collator, HanziUtils is disabled");
            sInstance = new HanziUtils(false);
            return sInstance;
        }
    }
